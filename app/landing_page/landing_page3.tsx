...
export function GrubzLandingPage() {
  ...
  return (
    <main className="bg-[#F9F9F9] dark:bg-[#121212] text-[#212121] dark:text-white font-sans transition-colors duration-300">
      ...
      <section className="min-h-screen flex flex-col justify-center items-center text-center px-4 bg-[#FFF0E6] dark:bg-[#2B2B2B]">
        <img src="/grubz-logo.png" alt="Grubz Logo" className="w-24 h-24 mb-6" />
        <h1 className="text-5xl md:text-7xl font-extrabold text-[#FF6D00] mb-4 tracking-wide">
          Savor the Taste
        </h1>
        <h2 className="text-3xl md:text-5xl font-semibold text-[#00BFA6] mb-6 wavy-text">
          of Convenience 🍽️
        </h2>
        <p className="text-lg md:text-xl max-w-2xl mb-8 text-gray-700 dark:text-gray-300">
          Your favorite bites — from campus cafeterias to your local go-to — all in one app.
        </p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <button className="bg-[#FF6D00] text-white px-6 py-3 rounded-lg font-semibold hover:bg-orange-600 transition">
            Join the Waitlist
          </button>
          <button className="border border-[#FF6D00] text-[#FF6D00] px-6 py-3 rounded-lg font-semibold hover:bg-orange-100 dark:hover:bg-[#3A3A3A] transition">
            Get Early Access
          </button>
        </div>
      </section>
